 ...
 */
 export function generateUnitFilename(shareOptions) {
  ...
  return `${trimmedSharePath.replace(new RegExp(path.sep,'g'), '-')}.mount`;
}
...
export function generateUnitPath(shareOptions) {
  ...
  return path.join(systemdUnitDirectory, generateUnitFilename(shareOptions));
}
...
export function generateUnitFilenames(options) {
  ...
  return shares.map(share => generateUnitFilename({ ...options, share }));
}